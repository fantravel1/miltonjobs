 ...
 * Generates individual HTML pages for each business
 */
...
const categoryIcons = {
  ...
  'Gym': '&#127947;', 'Auto Repair': '&#128295;', 'Auto Dealership': '&#128663;',
  'Real Estate': '&#127968;', 'Law Office': '&#9878;', 'School': '&#127979;',
  'Church': '&#9962;', 'Hotel': '&#127976;', 'Daycare': '&#128118;',
  'Veterinarian': '&#128054;', 'Construction': '&#127959;', 'Plumber': '&#128295;',
  'default': '&#128188;'
};
...
function generateBusinessPage(biz) {
...
<title>${escapeHtml(biz.name)} - ${escapeHtml(biz.category)} in Milton, ON | MiltonJobs.com</title>
<meta name="description" content="${escapeHtml(biz.name)} is a ${escapeHtml(biz.category)} located in ${escapeHtml(biz.neighborhood || 'Milton')}, Ontario. ${escapeHtml(biz.description || biz.services || '')} Find address, phone, and more on MiltonJobs.com.">
<meta name="robots" content="index, follow">
<link rel="canonical" href="https://miltonjobs.com/businesses/${slug}.html">

<meta property="og:type" content="place">
<meta property="og:title" content="${escapeHtml(biz.name)} - Milton, ON">
<meta property="og:description" content="${escapeHtml(biz.category)} in ${escapeHtml(biz.neighborhood || 'Milton')}, Ontario">
...
<script type="application/ld+json">
{
  ...
  "description": "${escapeHtml(biz.description || biz.services || '')}",
  "address": {
    "@type": "PostalAddress",
    "streetAddress": "${escapeHtml(biz.address || '')}",
    "addressLocality": "Milton",
    "addressRegion": "ON",
    "addressCountry": "CA"
  }${biz.phone ? `,
  "telephone": "${escapeHtml(biz.phone)}"` : ''}${biz.website ? `,
  "url": "${escapeHtml(biz.website)}"` : ''}
}
</script>
</head>
<body>
...
<section class="business-page-hero">
  <div class="job-hero-content">
    <div class="breadcrumb">
      <a href="/">Home</a> <span>&#8250;</span> <a href="/#directory">Directory</a> <span>&#8250;</span> <span>${escapeHtml(biz.name)}</span>
    </div>
    <span class="job-emoji">${icon}</span>
    <h1>${escapeHtml(biz.name)}</h1>
    <p class="job-subtitle">${escapeHtml(biz.category)} in ${escapeHtml(biz.neighborhood || 'Milton')}, Ontario</p>
  </div>
</section>
...
<div class="business-page-content">
  <div class="job-main">
    <section class="job-section">
      ...
      <p>${escapeHtml(biz.description || biz.services || biz.name + ' is a ' + biz.category + ' located in Milton, Ontario.')}</p>
    ...
    <section class="job-section">
      ...
      <div class="detail-grid">
        <div class="detail-item">
          ...
          <span>${escapeHtml(biz.address || 'Milton, Ontario')}</span>
        </div>
        ${biz.phone ? `<div class="detail-item">
          ...
          <a href="tel:${biz.phone}">${escapeHtml(biz.phone)}</a>
        </div>` : ''}
        ${biz.website ? `<div class="detail-item">
          ...
          <a href="${escapeHtml(biz.website)}" target="_blank" rel="noopener">${escapeHtml(biz.website.replace(/^https?:\/\//, ''))}</a>
        </div>` : ''}
      </div>
  ...
  <aside class="job-sidebar">
    <div class="sidebar-card">
      ...
      <div class="quick-facts">
        ...
        ${biz.rating ? `<div class="fact-row"><span class="fact-label">Rating</span><span class="fact-value">&#11088; ${biz.rating}</span></div>` : ''}
...
<footer class="site-footer" role="contentinfo">
  <div class="footer-bottom" style="padding:2rem 1rem;">
    <p>&copy; 2024&ndash;2026 MiltonJobs.com &mdash; Free Milton, Ontario Business Directory</p>
  </div>
</footer>

<script>
(function(){
  const t=localStorage.getItem('mj_theme')||(window.matchMedia('(prefers-color-scheme:dark)').matches?'dark':'light');
  document.documentElement.setAttribute('data-theme',t);
  document.addEventListener('DOMContentLoaded',function(){
    const tb=document.getElementById('themeToggle');
    if(tb){tb.addEventListener('click',function(){
      const nt=document.documentElement.getAttribute('data-theme')==='light'?'dark':'light';
      document.documentElement.setAttribute('data-theme',nt);localStorage.setItem('mj_theme',nt);
    });}
    const mb=document.getElementById('mobileMenuBtn'),mn=document.getElementById('mobileNav');
...
businesses.forEach(biz => {
  const slug = slugify(biz.name);
  const html = generateBusinessPage(biz);
  fs.writeFileSync(path.join(bizDir, `${slug}.html`), html, 'utf8');
  generated++;
});

console.log(`Generated ${generated} business detail pages in /businesses/`);
